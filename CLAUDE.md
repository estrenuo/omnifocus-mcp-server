...
- **A project's status cannot be set to done/dropped by assignment.** `project.status = "done status"` throws "Use the mark completed verb instead" (same for dropped). Use `project.markComplete()` / `project.markDropped()`. `active status` and `on hold status` *can* be assigned directly, and assigning `active status` reactivates a completed/dropped project. See `generateSetProjectStatusScript` in helpers.ts.
- **A new JXA project defaults to `sequential = true`.** `app.Project({name})` yields a sequential project unless you set otherwise, so `project.sequential` must be assigned explicitly (both `true` and `false`) or a "parallel" project silently comes back sequential.
...
- **Repetition rules cannot be set via direct JXA.** Assigning `task.repetitionRule = {...}` throws -1700 ("cannot convert types"), and the Omni Automation classes (`app.RecurrenceRule`, `app.RecurrenceType`, `app.RecurrenceDay`) do not exist in the JXA context. Set the rule through the Omni Automation bridge instead: `app.evaluateJavascript("... Task.byIdentifier(id).repetitionRule = new Task.RepetitionRule('FREQ=WEEKLY;INTERVAL=1;BYDAY=MO,TH', Task.RepetitionMethod.Fixed) ...")`. The rule string is a standard iCalendar (RFC 5545) RRULE. `Task.RepetitionMethod` members in the current OmniFocus build are **`Fixed`, `DeferUntilDate`, `DueDate`, `None`** — the older `StartAfterCompletion`/`DueAfterCompletion` names are `undefined` and silently fall back to `Fixed`. Reading back: `task.repetitionRule()` returns a record whose `.recurrence` (RRULE) and `.repetitionMethod` are plain string **properties** (no parentheses); calling `task.repetitionMethod()` as a function throws.
- **Completing or dropping a repeating task rolls it forward** to the next occurrence (a completed/dropped clone is created and the live instance is rescheduled). To truly stop a series (e.g. on drop), clear the rule first via the bridge (`Task.byIdentifier(id).repetitionRule = null`) and *then* `markDropped()`. `omnifocus_complete_task`/`batch_complete_task` do this on drop; `omnifocus_update_task` exposes `clearRecurrence: true` to turn recurring off without completing.
